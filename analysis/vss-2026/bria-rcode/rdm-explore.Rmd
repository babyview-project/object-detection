...
## Get long-form RDMs for plotting
```{r}
...
Order all the RDMs by the babyview clip one
```{r}
...
# Calculating and plotting category variablity

Correlate embeddings for all categories
```{r}
...
```
 ...
 Do this for both cliip and dino
```{R}
...
Get the correlations in long format for each cncoder
```{r}
...
Output most and least similar in clip
```{r}
most_similar <- corr_long_clip %>%
  filter(category1==category2) %>%
  slice_max(correlation, n=10)

least_similar <- corr_long_clip %>%
  filter(category1==category2) %>%
  slice_min(correlation, n=10)
```

Sanity check we did pearon's corr right in that function
```{r}
## sanity check
deer = as.numeric(embeddings_babyview_clip %>% filter(category=='deer'))
deer_things = as.numeric(embeddings_things_clip %>% filter(category=='deer'))
cor.test(deer[2:513], deer_things[2:513]) # funky because of the category column


sanity <- corr_long_clip %>%
  filter(category1=='deer') %>%
  filter(category1==category2)
```


Put together clip and dino similarity and get rid of the csecond category
```{r}
ordered_similarity <- corr_long_clip %>%
  ...
  select(-category2) 
```
Get some colors outputted by the CDI categories
```{r}
...
Plot similarity for clip vs dino by CDI category
```{r}
library(ggthemes)
