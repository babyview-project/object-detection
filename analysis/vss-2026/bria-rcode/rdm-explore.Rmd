...
corr_matrix = row_correlation_matrix(embeddings_babyview_clip, embeddings_things_clip)
dimnames(corr_matrix) <- list(categories, categories)
```
...
```{r}
ggplot(corr_long, aes(x = category1, y = category2, fill = correlation)) +
  geom_tile() +
  scale_fill_viridis_c(option = "magma", direction = -1) +
  coord_equal() +
   # ylim(0,1.8) +
  ...
  labs(
    x = NULL,
    y = NULL,
    fill = "Pearons corrleation distance",
    title = "THINGS vs BV Category similarity"
  )
```