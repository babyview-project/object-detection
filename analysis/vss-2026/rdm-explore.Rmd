...
makeLongRDM <- function(embeddings) {
...
```{R}
babyview_dino <- makeLongRDM(embeddings_babyview_dino)
babyview_clip <- makeLongRDM(embeddings_babyview_clip)
#
things_clip <- makeLongRDM(embeddings_things_clip)
things_dino <- makeLongRDM(embeddings_things_dino)
```
...
```{r}
ggplot(babyview_clip_ordered, aes(x = category1, y = category2, fill = cosine_distance)) +
  geom_tile() +
...
```{r}
ggplot(things_dino_ordered, aes(x = category1, y = category2, fill = cosine_distance)) +
  geom_tile() +
  scale_fill_viridis_c(option = "magma", direction = -1) +
  coord_equal() +
  # theme_minimal(base_size = 10) +
...
```{r}
ggplot(things_clip_ordered, aes(x = category1, y = category2, fill = cosine_distance)) +
  geom_tile() +
  scale_fill_viridis_c(option = "magma", direction = -1) +
  coord_equal() +
  # theme_minimal(base_size = 10) +
