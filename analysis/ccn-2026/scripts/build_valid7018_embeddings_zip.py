...
Creates a single zip under ``data/shared_data_ccn_2026/embeddings/`` with layout:

  valid7018_bv_embeddings.zip
    README.txt
    manifest.csv          # category, stem, clip_npy, dinov3_npy (in-archive paths)
    clip/{category}/{stem}.npy
    dinov3/{category}/{stem}.npy

Cohort definition matches ``compute_valid7018_local_global.py``.
...
~7,018 human-validated object crops (85 categories), each with paired:
  - CLIP ViT-B/32-style 512-d vectors (float16 on disk, shape (1, 512))
  - DINOv3 ViT-B/16 768-d vectors (float16, shape (1, 768))
...
Metrics on this cohort: analysis/ccn-2026/valid7018/

Rebuild archive:
  python analysis/ccn-2026/scripts/build_valid7018_embeddings_zip.py

Note: ``stem`` values are crop identifiers from the annotation pipeline (may encode
video/frame tokens). No subject_id column is included in manifest.csv.
"""
...
def main() -> int:
    ...
    for cat in sorted(clip_paths.keys()):
        dino_by_stem = {
            Path(p).stem.lower(): p for p in dino_paths.get(cat, [])
        }
        for clip_src in clip_paths[cat]:
            ...
            clip_arc = archive_member(cat, stem, "clip")
            dino_arc = archive_member(cat, stem, "dinov3")
            ...
            rows.append(
                {
                    ...
                    "clip_npy": clip_arc,
                    "dinov3_npy": dino_arc,
                    "_clip_src": str(clip_src),
                    "_dino_src": str(dino_src),
                }
            )
    ...
    with zipfile.ZipFile(tmp_zip, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
        ...
        for r in rows:
            zf.write(r["_clip_src"], arcname=r["clip_npy"])
            zf.write(r["_dino_src"], arcname=r["dinov3_npy"])
