...
```{r}
read_csv(file=here::here('frame_data/preprocessed_object_detections.csv'))
```
...
classes_per_age_by_subject <- test_dataframe %>%
  group_by(subject_id, age_mo) %>%
  summarize(num_classes = length(unique(class_name)))
...
age_model_no_slopes = glmer(data=for_glmer, cbind(num_detected, num_not_detected) ~ scale(age_mo) + (scale(age_mo) | subject_id) +   (1 | class_name), family='binomial')

anova(age_model, age_model_no_slopes)
```
