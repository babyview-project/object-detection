...
Similarity values varied substantially across categories, ranging from approximately 0.14 to 0.80 (average cosine similarity between BabyView vs Things, $\cos(\theta)=0.61$; Fig. \ref{fig:category_wise_cos_sim}). While we found relative convergence across the two feature spaces in which categories were more or less similar (Pearson's correlation between DINOv3 vs. CLIP category-wise similarity values, $r = 0.70$, $p < .01$), there was some divergence, particularly for categories with diverse exemplars where the DINO embeddings appeared to  capture more of this variability. For example, while all of the ``car" exemplars in THINGS were photographs of cars, the child's view contained many toys cars and depictions of cars -- leading to a lower correspondence between BabyView vs THINGS in the DINOv3 model (see \ref{fig:category_wise_cos_sim}). 
...
In fact, we found that the superordinate clusters were stronger in the BabyView dataset than in THINGS (see Figure \ref{fig:bv_things_rdm}). Quantifying within- versus between-cluster distances for CDI semantic groups showed stronger superordinate clustering in BabyView in both embedding spaces: CLIP $\Delta=0.056$, 95\% CI [0.022, 0.090], $p<.001$; DINOv3 $\Delta=0.112$, 95\% CI [0.074, 0.145], $p<.001$. Alignment with a binary CDI template was moderate (CLIP: $\rho_{BV}=0.353$, $\rho_{THINGS}=0.408$; DINOv3: $\rho_{BV}=0.344$, $\rho_{THINGS}=0.274$), and BV-vs-THINGS cluster-strength ranks were correlated (CLIP: $\rho=0.883$, $p=0.00159$; DINOv3: $\rho=0.733$, $p=0.0246$).

This effect was not uniform across CDI domains. In CLIP space, the largest between-minus-within separation in BabyView was observed for body parts ($\Delta=0.450$), furniture/rooms ($\Delta=0.401$), and vehicles ($\Delta=0.397$), whereas household items showed weaker separation ($\Delta=0.115$). The same pattern appeared in DINOv3, with strongest separation for vehicles ($\Delta=0.329$), body parts ($\Delta=0.300$), and furniture/rooms ($\Delta=0.298$), and again the weakest for household items ($\Delta=0.082$). Overall, these results suggest that despite high exemplar-level variability, category structure in infant visual experience remains strongly organized at a superordinate level, with greater redundancy than is captured in THINGS.

\begin{figure}
  \centering
  \includegraphics[width=.99\textwidth]{figures/results/bv_things_clip_dino_rdm_129cats.pdf}
  \caption{Representational dissimilarity matrix (RDM) for BabyView categories (CLIP, 129 categories, CLIP-filtered) (Spearman's $\rho = 0.55$, $p < .01$). Comparison with DINOv3 embeddings shows moderate correlation between BabyView and THINGS structure (Spearman's $\rho = 0.40$, $p < .01$).}
