...
```{R}
object_freq <- object_counts_by_age %>%
  group_by(class_name) %>%
...
# Overall top 50 proportional plot - HORIZONTAL
p1_prop_50 <- ggplot(top50_data_prop, 
  ...
  scale_fill_manual(values = c(
    "Animated" = "#8250A0",  # purple
    "Small" = "#FAA41A",     # orange
    "Big" = "#3A53A4",       # blue
    "Other" = "#C0BDBD",     # gray
    "Animated & Small" = "#8250A0",  # purple
    "Animated & Big" = "#8250A0",    # purple
    "Small & Big" = "#FAA41A"        # orange
  )) +
  scale_y_continuous(labels = scales::percent,
...
ggsave(here::here("analysis_plots/combined_top50_proportions_nolabels.png"),
       p1_prop_50,
       width = 20,  # Increased width for horizontal layout
       height = 16)  # Adjusted height for horizontal layout
...
# Create subject-level detection proportions using frame_id
```{r}
