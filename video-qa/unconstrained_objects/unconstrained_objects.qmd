---
title: "BabyView Objects unconstraiend detections"
format:
  html:
    toc: true
    toc-depth: 3
...
yoloe_objects_129 <- read.csv(here("data/overall_category_distribution_129.csv"))
yoloe_objects_85 <- read.csv(here("data/overall_category_distribution_85.csv"))
# Constants 
...
object_counts.bar.powerlaw <- function(df, input_n=50, input_title="Proportion of frames") {

  df_ranked <- df |>
    arrange(desc(proportion)) |>
    mutate(rank = row_number())

  sliced_df <- df_ranked |> slice_head(n = input_n)

  coverage = round(sum(sliced_df$video_count)/sum(df$video_count) * 100, 2)

  fit <- lm(log(proportion) ~ log(rank), data = df_ranked)

  ggplot(sliced_df, aes(x = rank, y = proportion)) +
    geom_point(color = "steelblue") +
    geom_line(color = "steelblue") +
    stat_function(
      fun = function(x) exp(coef(fit)[1]) * x^(coef(fit)[2]),
      color = "red",
      linewidth = 1
    ) +
    scale_x_log10() +
...
Top 50 by default.
```{r obj-with-frames, fig.width=20, fig.height=6}
...
## Types of toys
```{r toys, fig.width=20, fig.height=6}
toys <- object_counts |>
...
Some YOLOe cleaning
```{r}
yoloe_objects_129_comp <- yoloe_objects_129 |>
  filter(!is.na(category)) |>
  distinct(category, .keep_all=TRUE) |>
  mutate(
    parsed = map(category, ~ spacy_parse(.x, lemma = TRUE, pos = TRUE)),
    ...
    class_name = case_when(category == "teddybear" ~ "teddy bear",
                          TRUE ~ category)
  ) |>
  select(-parsed) 

yoloe_objects_85_comp <- yoloe_objects_85 |>
  filter(!is.na(category)) |>
  distinct(category, .keep_all=TRUE) |>
  mutate(
    parsed = map(category, ~ spacy_parse(.x, lemma = TRUE, pos = TRUE)),
...
yoloe_objects_all <- yoloe_objects_129_comp |>
  mutate(
    category = if_else(
      class_name %in% yoloe_objects_85_comp$class_name,
      "filtered",
      "total"
    )
  )
...
```{r missing, fig.width=20, fig.height=6}
...
## missing YOLOe words
```{r}
yoloe_and_vqa <- object_counts |> inner_join(yoloe_objects_all |> distinct(class_name, .keep_all=TRUE) |> rename(yoloe_proportion = proportion), by=c("cleaned_lemma"="class_name"))
yoloe_only_words <- yoloe_objects_all |> anti_join(yoloe_and_vqa, by=c("class_name"="cleaned_lemma")) |> rename(object=class_name)
...
yoloe_and_vqa_labeled <- yoloe_and_vqa %>%
  ...
  mutate(category = ifelse(category == "filtered", "high-precision subset", category))

# Plot
ggplot(yoloe_and_vqa_labeled, aes(x = log_prop, y = log_yoloe, color = category)) +
  geom_point(size = 5, alpha = 0.5) +
  
  # black line: all data
  geom_smooth(
    data = yoloe_and_vqa_labeled,
    aes(x = log_prop, y = log_yoloe),
    method = "lm",
    se = FALSE,
    color = "gray",
    size = 1.2,
  ...
  # blue line: high-precision subset only
  geom_smooth(
    data = subset(yoloe_and_vqa_labeled, category == "high-precision subset"),
    aes(x = log_prop, y = log_yoloe),
    method = "lm",
    se = FALSE,
    color = "#1f78b4",
  ...
  ggrepel::geom_label_repel(
    ...
    aes(label = cleaned_lemma, color = category),
    size = 5,
    max.overlaps = 20,
    box.padding = 0.4,
    point.padding = 0.3,
  ...
  scale_color_manual(values = c("high-precision subset" = "#1f78b4")) +
  ...
  ylab("log(proportion) from automated pipeline") +
  xlab("log(proportion) from video question-answering model") +
...
ggsave(here("data/figures/vqa_comparison.png"))
```


```{r}
df_ranked <- yoloe_and_vqa |>
  filter(category == "filtered") |>
  rename(object = cleaned_lemma) |>
  arrange(desc(proportion)) |>
  mutate(rank = row_number())
